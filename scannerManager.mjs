...
import { CARLSJR_ABI, CARLSJR_ADDRESS } from './dynamicAbi.mjs';
...
function scheduleBatchSend(loanToken, pairNameForLog) {
  ...
  const timer = setTimeout(async () => {
    ...
    // Use the first payload's amountIn
    const baseAmountIn = BigInt(payloads[0].amountIn);

    const SAFETY_MARGIN_BPS = 300n;
    const AAVE_FEE_BPS      = 5n;
    const BPS_DIV           = 10000n;

    const flashFee = (baseAmountIn * AAVE_FEE_BPS) / BPS_DIV;
    const profitWithMargin = (totalMinProfit * (BPS_DIV - SAFETY_MARGIN_BPS)) / BPS_DIV;
    ...
    for (let i = 0; i < 3; i++) {
      try {
        const tx  = await carlsJrContract.executePreciseArbitrage(params, txOpts);
  ...
  function spawnNext() {
    ...
    w.on('message', async (msg) => {
      if (msg.type === 'ARBITRAGE_FOUND') {
        ...
        pendingBatches.set(loanToken, entry);
        scheduleBatchSend(loanToken, 'multi');
